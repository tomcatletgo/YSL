...
public class OpenOfficePDFConverter implements PDFConverter {
	...
	public void convert2PDF(String inputFile, String pdfFile, String extend) {
		...
		if (extend.equals("txt")) {
			...
			if (new File(odtFile).exists()) {
				core.util.LogUtil.info("odt文件已存在！");
				inputFile = odtFile;
			} else {
				try {
					...
					core.util.LogUtil.info("Odt文档不存在！");
					e.printStackTrace();
				}
			}
		...
		//pdfFile = pdfFile.replaceAll(" ", "").replaceAll("　", "");
		 core.util.LogUtil.info("进行文档转换转换:" + inputFile + " --> " + pdfFile);
		...
		try {
			...
			core.util.LogUtil.info(e.getMessage());
		}
		
		stopService();
	    core.util.LogUtil.info("进行文档转换转换---- 结束----");
	}
	...
	public static void startService() {
		...
		try {
			...
			core.util.LogUtil.info("office转换服务启动成功!");
		} catch (Exception ce) {
			core.util.LogUtil.info("office转换服务启动失败!详细信息:" + ce);
		}
	}

	public static void stopService() {
		core.util.LogUtil.info("关闭office转换服务....");
		if (officeManager != null) {
			officeManager.stop();
		}
		core.util.LogUtil.info("关闭office转换成功!");
	}
}
