...
import core.common.dao.IGenericBaseCommonDao;
import core.common.dao.jdbc.JdbcDao;
import core.common.exception.BusinessException;
import core.common.hibernate.qbc.CriteriaQuery;
import core.common.hibernate.qbc.DetachedCriteriaUtil;
import core.common.hibernate.qbc.HqlQuery;
import core.common.hibernate.qbc.PageList;
import core.common.hibernate.qbc.PagerUtil;
import core.common.model.json.DataGridReturn;
import core.util.MyBeanUtils;
import core.util.ToEntityUtil;
import core.util.oConvertUtils;
import tag.vo.datatable.DataTableReturn;
...
public abstract class GenericBaseCommonDao<T, PK extends Serializable> implements IGenericBaseCommonDao {
	...
	private <T> void getProperty(final Class entityName) {
		final ClassMetadata cm = sessionFactory.getClassMetadata(entityName);
		final String[] str = cm.getPropertyNames(); // 获得该类所有的属性名称
		for (int i = 0; i < str.length; i++) {
			final String property = str[i];
			final String type = cm.getPropertyType(property).getName(); // 获得该名称的类型
	...
	public <T> T findUniqueByProperty(final Class<T> entityClass,
	 ...
	 * 按属性查找对象列表.
	 */
	public <T> List<T> findByProperty(final Class<T> entityClass,
			final String propertyName, final Object value) {
		Assert.hasText(propertyName);
	 ...
	 * @param pojo
	 */
	public <T> void deleteEntityById(final Class entityName, final Serializable id) {
	 ...
	 * @param entitys
	 */
	public <T> void deleteAllEntitie(final Collection<T> entitys) {
	 ...
	 * @return
	 */
	public <T> T getEntity(final Class entityName, final Serializable id) {

		final T t = (T) getSession().get(entityName, id);
		if (t != null) {
			getSession().flush();
		}
		return t;
	 ...
	 * @param pojo
	 */
	public <T> void updateEntitie(final T pojo) {
	 ...
	 * @param pojo
	 */
	public <T> void updateEntitie(final String className, final Object id) {
	 ...
	 * 根据主键更新实体
	 */
	public <T> void updateEntityById(final Class entityName, final Serializable id) {
	...
	public List<T> findByQueryString(final String query) {
		...
		final Query queryObject = getSession().createQuery(query);
		final List<T> list = queryObject.list();
		if (list.size() > 0) {
			getSession().flush();
		}
		return list;
	 ...
	 * @return
	 */
	public <T> T singleResult(final String hql) {
		T t = null;
		final Query queryObject = getSession().createQuery(hql);
		final List<T> list = queryObject.list();
		if (list.size() == 1) {
			getSession().flush();
			t = list.get(0);
	 ...
	 * @return
	 */
	public Map<Object, Object> getHashMapbyQuery(final String hql) {

		final Query query = getSession().createQuery(hql);
		final List list = query.list();
		final Map<Object, Object> map = new HashMap<Object, Object>();
		for (final Iterator iterator = list.iterator(); iterator.hasNext();) {
			final Object[] tm = (Object[]) iterator.next();
	...
	public int updateBySqlString(final String query) {
		...
		final Query querys = getSession().createSQLQuery(query);
	...
	public List<T> findListbySql(final String sql) {
		final Query querys = getSession().createSQLQuery(sql);
		return querys.list();
	}
	 ...
	 * @return
	 */
	private <T> Criteria createCriteria(final Class<T> entityClass, final boolean isAsc,
			final Criterion... criterions) {
		final Criteria criteria = createCriteria(entityClass, criterions);
		if (isAsc) {
	 ...
	 * @return
	 */
	public <T> List<T> findByPropertyisOrder(final Class<T> entityClass,
			final String propertyName, final Object value, final boolean isAsc) {
		Assert.hasText(propertyName);
	 ...
	 * @return 符合条件的唯一对象.
	 */
	public <T> T findUniqueBy(final Class<T> entityClass, final String propertyName,
			final Object value) {
		Assert.hasText(propertyName);
	 ...
	 * @return
	 */
	public <T> int batchInsertsEntitie(final List<T> entityList) {
		int num = 0;
	...
	public List findByExample(final String entityName,
		...
		final Criteria executableCriteria = (entityName != null ? getSession()
	 ...
	 * 调用存储过程
	 */
	public void callableStatementByName(final String proc) {
	}
	 ...
	 * @return
	 */
	public int getCount(final Class<T> clazz) {

		final int count = DataAccessUtils.intResult(getSession().createQuery(
	...
	public PageList getPageList(final CriteriaQuery cq, final boolean isOffset) {
		...
		final Criteria criteria = cq.getDetachedCriteria().getExecutableCriteria(
				getSession());
		...
		final int curPageNO = PagerUtil.getcurPageNo(allCounts, cq.getCurPage(),
				pageSize);// 当前页
		final int offset = PagerUtil.getOffset(allCounts, curPageNO, pageSize);
		String toolBar = "";
		if (isOffset) {// 是否分页
	...
	public PageList getPageListBySql(final HqlQuery hqlQuery,
		...
		final Query query = getSession().createSQLQuery(hqlQuery.getQueryString());

		// query.setParameters(hqlQuery.getParam(), (Type[])
		// hqlQuery.getTypes());
		final int allCounts = query.list().size();
		final int curPageNO = hqlQuery.getCurPage();
		final int offset = PagerUtil.getOffset(allCounts, curPageNO,
	...
	public PageList getPageList(final HqlQuery hqlQuery,
		...
		final Query query = getSession().createQuery(hqlQuery.getQueryString());
		if (needParameter) {
		...
		final int offset = PagerUtil.getOffset(allCounts, curPageNO,
				hqlQuery.getPageSize());
		final String toolBar = PagerUtil.getBar(hqlQuery.getMyaction(), allCounts,
	 ...
	 * @return
	 */
	public List<T> getListByCriteriaQuery(final CriteriaQuery cq, final Boolean ispage) {
		final Criteria criteria = cq.getDetachedCriteria().getExecutableCriteria(
				getSession());
		// 判断是否有排序字段
		if (cq.getOrdermap() != null) {
	 ...
	 */
	public List<Map<String, Object>> findForJdbc(String sql, final int page, final int rows) {
		// 封装分页SQL
	 ...
	 */
	public <T> List<T> findObjForJdbc(String sql, final int page, final int rows,
			final Class<T> clazz) {
		final List<T> rsList = new ArrayList<T>();
		// 封装分页SQL
		sql = JdbcDao.jeecgCreatePageSql(sql, page, rows);
	 ...
	 */
	public List<Map<String, Object>> findForJdbcParam(String sql, final int page,
			final int rows, final Object... objs) {
		// 封装分页SQL
		sql = JdbcDao.jeecgCreatePageSql(sql, page, rows);
	 ...
	 */
	public Long getCountForJdbc(final String sql) {
	 ...
	 * 
	 */
	public Long getCountForJdbcParam(final String sql, final Object[] objs) {
	...
	public Integer executeSql(final String sql, final List<Object> param) {
	...
	public Integer executeSql(final String sql, final Map<String, Object> param) {
		return this.namedParameterJdbcTemplate.update(sql, param);
	}
	public Object executeSqlReturnKey(final String sql, final Map<String, Object> param) {
		Object keyValue = null;
		try{
			final KeyHolder keyHolder = new GeneratedKeyHolder(); 
			final SqlParameterSource sqlp  = new MapSqlParameterSource(param);
	...
	public Integer countByJdbc(final String sql, final Object... param) {
	 ...
	 * @return
	 */
	public <T> List<T> findHql(final String hql, final Object... param) {
	...
	public <T> List<T> pageList(final DetachedCriteria dc, final int firstResult,
			final int maxResult) {
		final Criteria criteria = dc.getExecutableCriteria(getSession());
		criteria.setResultTransformer(CriteriaSpecification.ROOT_ENTITY);
	 ...
	 * 离线查询
	 */
	public <T> List<T> findByDetached(final DetachedCriteria dc) {
