...
public class AuthInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object) throws Exception {
		...
		if (excludeUrls.contains(requestPath) || requestPath.indexOf("rest/") != -1) {
			return true;
		} else {
			if (client != null && client.getUser() != null) {
				//查询当前用户关联角色
				List<TSRole> listRole = roleService.selJurisdictionAdmin(client.getUser().getId());
				String admin = "";
				for (TSRole tsRole : listRole) {
					if(tsRole.getRoleCode().equals("admin"))
					{
						admin = tsRole.getRoleCode();
					}
				}
