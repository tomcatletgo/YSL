...
public class LogController extends BaseController {
	...
	public void datagrid(HttpServletRequest request,
		...
		String beginupdtime = request.getParameter("beginupdtime");
		String endupdtime = request.getParameter("endupdtime");
		...
		try {
			...
			if (beginupdtime != null && beginupdtime.length() > 0) {
				map.put("beginupdtime", sdf.parse(beginupdtime));
			}
			if (endupdtime != null && endupdtime.length() > 0) {
				map.put("endupdtime", sdf.parse(endupdtime));
			}
		...
		// try {
		// if (beginupdtime != null && beginupdtime.length() > 0) {
		// Date date = sdf.parse(beginupdtime);
		// cq.ge("operatetime", date);
		// }
		// if (endupdtime != null && endupdtime.length() > 0) {
		// Date date = sdf.parse(endupdtime);
		// cq.le("operatetime", date);
		// }
		...
		// }
		// if (accessType == null || accessType.equals("0")) {
		// } else {
		// cq.eq("accessType", oConvertUtils.getShort(accessType));
		// }
		// cq.addOrder("operatetime", SortDirection.desc);
		// cq.add();
		
		
		List<TSLog> data = this.logService.getLogList(map);
		int total = this.logService.getLogCount(map);
	...
	public ModelAndView userBroswer(String reportType,
	...
	public List<Highchart> getBroswerBar(HttpServletRequest request,
		...
		Long count = systemService
				.getCountForJdbc("SELECT COUNT(1) FROM T_S_Log WHERE 1=1");
